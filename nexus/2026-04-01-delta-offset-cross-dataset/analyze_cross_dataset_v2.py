...
def main():
    ...
    for dataset in datasets:
        ...
        try:
            ...
            all_results[dataset] = {"N": int(N), "D": int(D), "results": results}
            ...
            print(f"\n平均 SNR: Delta={avg_delta_snr:.4f}, Offset={avg_offset_snr:.4f}, 比值={avg_snr_ratio:.2f}x")
    ...
    print(f"\n{'数据集':<15} {'D':<8} {'Delta SNR':<12} {'Offset SNR':<12} {'SNR比值':<10}")
    print("-"*70)
    
    for dataset in datasets:
        if "summary" in all_results.get(dataset, {}):
            s = all_results[dataset]["summary"]
            print(f"{dataset.upper():<15} {all_results[dataset]['D']:<8} {s['avg_delta_snr']:<12.4f} {s['avg_offset_snr']:<12.4f} {s['avg_snr_ratio']:<10.2f}x")
    
    # 保存结果
    output = {"timestamp": datetime.now().isoformat(), "datasets": all_results}
