#!/usr/bin/env python3
"""RIFT-GT R1 position/role-encoded ROCC probe for C-LEG3 response matrices.
...
def make_r1_variants():
    """Large but controlled R1 hyperparameter grid.

    R1 changes the information boundary relative to R0: it keeps the same
    C-LEG3 response matrix and ROCC objective, but gives the reader token
    identity features. AUC/AP remain diagnostic-only.
    """
    variants = {}
    # Main grid: 3 identity modes × 2 poolers × 2 learning rates × 3 unlabeled weights = 36 variants.
    for pos_mode in ["rowcol", "rowcol_rank", "rowcol_role"]:
        for pooling in ["mean", "attn"]:
            for lr_name, lr in [("lr1e3", 1e-3), ("lr3e4", 3e-4)]:
                for lu_name, lu in [("lu10", 0.10), ("lu25", 0.25), ("lu50", 0.50)]:
                    name = f"r1_{pos_mode}_{pooling}_{lr_name}_{lu_name}"
                    variants[name] = {
                        "report_codename": f"RIFT-R1-{pos_mode}-{pooling}-{lr_name}-{lu_name}",
                        "definition": f"R1 {pos_mode} token identity, {pooling} pooling, ROCC lambda_unlabeled={lu}, AdamW lr={lr}.",
                        "changes": {"pos_mode": pos_mode, "pooling": pooling, "rift_lr": lr, "lambda_trimmed_unlabeled": lu, "rift_weight_decay": 1e-4},
                    }
    # Capacity probes: only on the strongest prior-like setting to avoid exploding grid too much.
    for dim, ffn in [(64, 128), (80, 160)]:
        for pooling in ["mean", "attn"]:
            name = f"r1_rowcol_rank_{pooling}_dim{dim}_lr1e3_lu25"
            variants[name] = {
                "report_codename": f"RIFT-R1-rowcol_rank-{pooling}-dim{dim}-lr1e3-lu25",
                "definition": f"R1 row/column/rank identity, {pooling} pooling, wider reader dim={dim}, ROCC lambda_unlabeled=0.25, AdamW lr=1e-3.",
                "changes": {"pos_mode": "rowcol_rank", "pooling": pooling, "rift_dim": dim, "rift_ffn_dim": ffn, "rift_lr": 1e-3, "lambda_trimmed_unlabeled": 0.25, "rift_weight_decay": 1e-4},
            }
    return variants


RIFT_VARIANTS = make_r1_variants()
...
def run_one(cli_args, rift_variant, seed, device):
    ...
    row = {
        ...
        "protocol": {
            ...
            "position_encoding": f"R1 pos_mode={args.pos_mode}; scalar response-entry value plus controlled row/column/rank/role embeddings; global scalar normalization from known normals",
...
def main():
    ...
    def snapshot(status="running", current=None):
        atomic_write_json(args.progress_out, {
            "status": status,
            "probe": "rift_gt_r1_pos_rocc_probe",
            "done": done,
            "total": total,
            "variants": variants,
