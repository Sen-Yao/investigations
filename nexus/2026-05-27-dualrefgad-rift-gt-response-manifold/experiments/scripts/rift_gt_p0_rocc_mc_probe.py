#!/usr/bin/env python3
"""RIFT-GT P0 ROCC-MC probe for C-LEG3 response matrices.

Protocol:
- Freeze C-LEG3 / old_exact_080_regime response matrix construction.
- Tokenize each response-matrix entry as scalar value plus controlled row/column/rank/role embeddings.
- Train a small Set/Transformer reader with ROCC-MC (multi-center robust one-class compactness):
  known-normal multi-prototype compactness + trimmed majority-unlabeled compactness + light anti-collapse regularizer.
...
def make_p0_variants():
    """P0 ROCC-MC grid: test whether single-center ROCC compressed normal heterogeneity.

    Keep C-LEG3 response matrix and R1 token identity fixed; change only the
    objective/score from one normal center to multiple normal prototypes.
    """
    variants = {}
    for k in [2, 4, 8]:
        for pooling in ["mean", "attn"]:
            name = f"p0_rocc_mc_k{k}_{pooling}_rowcol_rank_lu25"
            variants[name] = {
                "report_codename": f"RIFT-P0-ROCC-MC-k{k}-{pooling}",
                "definition": f"P0 ROCC-MC with {k} known-normal prototypes, rowcol_rank token identity, {pooling} pooling, lambda_unlabeled=0.25.",
                "changes": {"pos_mode": "rowcol_rank", "pooling": pooling, "mc_prototypes": k, "rift_lr": 1e-3, "lambda_trimmed_unlabeled": 0.25, "rift_weight_decay": 1e-4},
            }
    return variants

RIFT_VARIANTS = make_p0_variants()
...
def run_one(cli_args, rift_variant, seed, device):
    ...
    rift_metrics = metric_block(pack["labels_np"], pack["idx_test"], {
        "rift_rocc_mc_energy": rift_score,
        "margin": pack["margin"],
    ...
    row = {
        ...
        "comparisons": {
            "rift_minus_mat_mean_auc": None if rift_metrics["rift_rocc_mc_energy"]["auc"] is None else float(rift_metrics["rift_rocc_mc_energy"]["auc"] - pack["baseline_metrics"]["mat_mean"]["auc"]),
            "rift_minus_margin_auc": None if rift_metrics["rift_rocc_mc_energy"]["auc"] is None else float(rift_metrics["rift_rocc_mc_energy"]["auc"] - pack["baseline_metrics"]["margin"]["auc"]),
    ...
    print(json.dumps({"stage": "rift_seed_done", "variant": rift_variant, "seed": seed, "rift_auc": rift_metrics["rift_rocc_mc_energy"]["auc"], "mat_mean_auc": pack["baseline_metrics"]["mat_mean"]["auc"]}, ensure_ascii=False), flush=True)
    return row
...
def summarize(rows):
    return {
        "n_rows": len(rows),
        "rift_auc": mean_std([r["rift_metrics"]["rift_rocc_mc_energy"]["auc"] for r in rows]),
        "rift_ap": mean_std([r["rift_metrics"]["rift_rocc_mc_energy"]["ap"] for r in rows]),
...
def main():
    ...
    def snapshot(status="running", current=None):
        atomic_write_json(args.progress_out, {
            "status": status,
            "probe": "rift_gt_p0_rocc_mc_probe",
            "done": done,
            "total": total,
            "variants": variants,
    ...
    payload = {
        ...
        "probe": "rift_gt_p0_rocc_mc_probe",
        "method_family": "RIFT-GT / P0 ROCC-MC",
        "protocol": {
            ...
            "loss": "ROCC-MC = min-distance-to-known-normal-prototypes compactness + trimmed majority-unlabeled compactness + light anti-collapse variance floor",
