#!/usr/bin/env python3
"""Stage-1C high-dimensional DualRef relation MLP probe.

Encoder/reference frozen. MLP consumes high-dimensional relation inputs rather
than scalar margin calibration features. Training uses pairwise ranking loss on
normal-only matched vs mismatched relation tuples.
"""
...
def main():
    ...
    ap.add_argument('--input_mode', choices=['ud','ud_prod_absdiff','h_rn_ra_ud'], default='ud_prod_absdiff')
    ap.add_argument('--neg_mode', choices=['N1_context','N2_direction','mixed_N1_N2'], default='N2_direction')
    ...
    in_dim={'ud':2,'ud_prod_absdiff':4,'h_rn_ra_ud':7}[args.input_mode]*args.embedding_dim
    ...
    if args.wandb:
        import wandb
        run=wandb.init(project='DualRefGAD', entity='HCCS', config=vars(args), name=f'stage1_relation_mlp_{args.input_mode}_{args.neg_mode}_s{args.seed}')
    ...
    for epoch in range(args.num_epoch+1):
        if epoch>0:
            ...
            c=normal_np[rng.permutation(n)]; same=c==normal_np
            if np.any(same): c[same]=normal_np[(np.where(same)[0]+1)%n]
            x_pos,_=build_relation_features(emb,normal_refs_t,anom_refs_t,normal_np,args.input_mode)
            if args.neg_mode=='N1_context':
                x_neg,_=build_relation_features(emb,normal_refs_t,anom_refs_t,normal_np,args.input_mode,rn_idx=c,ra_idx=c)
            elif args.neg_mode=='N2_direction':
                x_neg,_=build_relation_features(emb,normal_refs_t,anom_refs_t,normal_np,args.input_mode,rn_idx=None,ra_idx=c)
            else:
                half=n//2; c1=c[:half]; c2=c[half:]
                x_n1,_=build_relation_features(emb,normal_refs_t,anom_refs_t,normal_np[:half],args.input_mode,rn_idx=c1,ra_idx=c1)
                x_n2,_=build_relation_features(emb,normal_refs_t,anom_refs_t,normal_np[half:],args.input_mode,rn_idx=None,ra_idx=c2)
                x_neg=torch.cat([x_n1,x_n2],dim=0)
            s_pos=head(x_pos); s_neg=head(x_neg)
            loss=F.softplus(s_pos - s_neg + args.gamma).mean()
        ...
        auc,apv=safe_auc(labels_np[idx_test],s_all[idx_test])
        row={'epoch':epoch,'loss':float(loss.detach().cpu().item()),'test_auc':auc,'test_ap':apv,'spearman_score_margin':safe_spearman(s_all[idx_test],margin0_np[idx_test]),'top1_ratio':top_ratio(labels_np[idx_test],s_all[idx_test],0.01),'top5_ratio':top_ratio(labels_np[idx_test],s_all[idx_test],0.05),'margin_auc':margin_auc,'margin_ap':margin_ap}
        last=row
        if auc+apv > best['auc']+best['ap']:
            best={'auc':auc,'ap':apv,'epoch':epoch,'spearman':row['spearman_score_margin'],'top1':row['top1_ratio'],'top5':row['top5_ratio']}
        if run and (epoch%10==0 or epoch==args.num_epoch):
            import wandb; wandb.log(row, step=epoch)
    result={'status':'stage1_relation_mlp_pairwise','dataset':args.dataset,'seed':args.seed,'config':vars(args),'purity':pur,'margin_baseline':{'auc':margin_auc,'ap':margin_ap,'top1_ratio':top_ratio(labels_np[idx_test],margin0_np[idx_test],0.01),'top5_ratio':top_ratio(labels_np[idx_test],margin0_np[idx_test],0.05)},'best':best,'last':last,'time_sec':time.time()-t0}
    out=Path(args.out) if args.out else root/'outputs/stage1_probe/stage1_relation_mlp_pairwise.json'
    ...
    if run:
        import wandb
        wandb.summary.update({'best_test_auc':best['auc'],'best_test_ap':best['ap'],'best_epoch':best['epoch'],'test_auc':best['auc'],'test_ap':best['ap'],'margin_auc':margin_auc,'margin_ap':margin_ap})
        run.finish()
