#!/usr/bin/env python3
"""Stage-1D high-dimensional DualRef relation MLP margin-regression probe.

Encoder/reference frozen. MLP consumes high-dimensional relation inputs and is trained to regress the closed-form DualRef margin. This is a diagnostic: it tests expression/optimization capacity without using anomaly labels.
"""
...
def main():
    ...
    for epoch in range(args.num_epoch+1):
        ...
        last=row
        # Select by margin reconstruction agreement, not anomaly labels.
        if row['spearman_score_margin'] > best.get('spearman', -2):
    ...
    result={'status':'stage1_relation_mlp_margin_regression','dataset':args.dataset,'seed':args.seed,'config':vars(args),'purity':pur,'margin_baseline':{'auc':margin_auc,'ap':margin_ap,'top1_ratio':top_ratio(labels_np[idx_test],margin0_np[idx_test],0.01),'top5_ratio':top_ratio(labels_np[idx_test],margin0_np[idx_test],0.05)},'best':best,'last':last,'time_sec':time.time()-t0}
