#!/usr/bin/env python3
"""Stage-2 Likelihood-Ratio Probe with ||d|| conditioning.

Core idea: Train conditional density p(phi | ||d||) on normal nodes only.
Anomaly score = likelihood ratio: log p(phi | ||d||_self) - log p(phi | ||d||_normal_mean)

This respects DualRef geometry: anomaly direction enters scoring via conditioning,
not via pseudo-anomaly generation or latent absorption.
"""
...
def build_components(emb, normal_refs, anom_refs, node_idx, rn_idx=None, ra_idx=None):
    ...
    d_norm=torch.norm(d, p=2, dim=1)  # ||d|| as conditioning
    return h,rn,ra,u,d,margin,d_norm


def build_relation_features(emb, normal_refs, anom_refs, node_idx, input_mode, rn_idx=None, ra_idx=None):
    h,rn,ra,u,d,margin,d_norm=build_components(emb, normal_refs, anom_refs, node_idx, rn_idx, ra_idx)
...
class ConditionScaler(nn.Module):
    """Scale conditioning variable to reasonable range."""
    def __init__(self, mean, std):
        super().__init__()
        self.register_buffer('mean', mean)
        self.register_buffer('std', std.clamp_min(1e-6))
...
def main():
    ...
    ap.add_argument('--lr_strategy', choices=['self_vs_mean','self_vs_low','high_vs_low'], default='self_vs_mean')
    ...
    # Build relation features and conditioning variable ||d||
    with torch.no_grad():
        x_all, margin0, d_norm_all = build_relation_features(emb, normal_refs_t, anom_refs_t, all_nodes, args.input_mode)

    x_train = x_all[torch.as_tensor(normal_idx, dtype=torch.long, device=device)]
    d_norm_train = d_norm_all[torch.as_tensor(normal_idx, dtype=torch.long, device=device)]
    ...
    # Standardize conditioning variable ||d||
    c_mean = d_norm_train.mean()
    c_std = d_norm_train.std().clamp_min(1e-6)
    c_scaler = ConditionScaler(c_mean, c_std).to(device)
    c_train = c_scaler(d_norm_train).detach()
    c_all = c_scaler(d_norm_all).detach()
    ...
    if args.wandb:
        ...
        run = wandb.init(
            ...
            entity='HCCS',
            config=vars(args),
            name=f'stage2_lr_{args.lr_strategy}_{args.input_mode}_h{args.hidden}_l{args.flow_layers}_s{args.seed}'
        )
        wandb.summary.update(pur)

    # Train conditional density p(phi | ||d||)
    density = ConditionalRealNVP(xs_train.shape[1], args.hidden, args.flow_layers, cond_dim=1).to(device)
    opt = torch.optim.AdamW(density.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    best_score = None
    best_nll = 1e9
    best_epoch = -1
    c_normal_mean = c_train.mean()  # reference point for likelihood ratio

    for epoch in range(args.num_epoch + 1):
        if epoch > 0:
            density.train()
            opt.zero_grad()
            nll = (-density.log_prob(xs_train, c_train.unsqueeze(1))).mean()
            nll.backward()
            opt.step()

        density.eval()
        with torch.no_grad():
            train_nll = float((-density.log_prob(xs_train, c_train.unsqueeze(1))).mean().item())

            # Compute likelihood ratio score
            # s_i = log p(phi_i | ||d||_i) - log p(phi_i | ||d||_normal_mean)
            log_p_self = density.log_prob(xs_all, c_all.unsqueeze(1))
            log_p_mean = density.log_prob(xs_all, c_normal_mean.expand(xs_all.shape[0]).unsqueeze(1))
    ...
    result = {
        'status': 'stage2_likelihood_ratio_probe',
    ...
    out = Path(args.out) if args.out else root / 'outputs/stage2_probe' / f'stage2_lr_probe_s{args.seed}.json'
