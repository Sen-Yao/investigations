#!/usr/bin/env python3
"""Stage-1E margin regression MSE probe with raw/normalized relation inputs.
...
def main():
    ...
    for epoch in range(args.num_epoch+1):
        ...
        if row['test_mse'] < best.get('mse', 1e9):
            best={'mse':mse,'mae':mae,'epoch':epoch,'spearman':row['spearman_score_margin'],'top1':row['top1_ratio'],'top5':row['top5_ratio']}
        if run and (epoch%10==0 or epoch==args.num_epoch):
            import wandb; wandb.log(row, step=epoch)
    result={'status':'stage1_margin_regression_mse_probe','dataset':args.dataset,'seed':args.seed,'config':vars(args),'purity':pur,'margin_baseline':{'mse':float(F.mse_loss(torch.as_tensor(margin0_np[idx_test],device=device), torch.as_tensor(margin0_np[idx_test],device=device)).item()),'mae':0.0,'spearman':1.0,'top1_ratio':top_ratio(labels_np[idx_test],margin0_np[idx_test],0.01),'top5_ratio':top_ratio(labels_np[idx_test],margin0_np[idx_test],0.05)},'best':best,'last':last,'time_sec':time.time()-t0}
    ...
    if run:
        import wandb
        wandb.summary.update({'best_test_mse':best['mse'],'best_test_mae':best['mae'],'best_epoch':best['epoch'],'best_spearman':best['spearman']})
        run.finish()
