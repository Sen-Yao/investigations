#!/usr/bin/env python3
"""G2 pseudo-anomaly scale calibration probe for DualRefGAD.
...
space, then sweeps perturbation scale to target G2 difficulty.
...
ROUTE25_SCRIPT_DIR = Path(os.environ.get(
    "ROUTE25_SCRIPT_DIR",
    str(_LOCAL_SCRIPT_DIR if (_LOCAL_SCRIPT_DIR / "route25_leg3_response_matrix_decomposition_probe.py").exists() else Path("/home/openclawvm/investigations/nexus/2026-05-19-dualrefgad-route25-matrix-autoencoder/experiments/scripts"))
))
...
def make_pseudo(norm_x, mu, sd, rng, noise_scale=1.0, tail_boost=1.0, strategy="v0_mixed"):
    ...
    if strategy == "normal_shell":
        ...
        direction = z / norm
        target_radius = np.quantile(norm, 0.75) + rng.uniform(0.25, 0.85, size=(n, 1)).astype(np.float32) * noise_scale
    ...
    if strategy == "tail_quantile_match":
        ...
        for i in range(n):
            ...
            idx = rng.choice(d, size=k, replace=False)
            alpha = rng.uniform(0.2, 1.0, size=k).astype(np.float32) * tail_boost
            pseudo[i, idx] = q75[idx] + (q95[idx] - q75[idx]) * alpha
        return _clip01(pseudo)
...
def run_one(args, variant, pseudo_strategy, pseudo_scale, seed, device):
    set_seed(seed)
    rng = np.random.default_rng(seed + 20260522 + (abs(hash((pseudo_strategy, float(pseudo_scale)))) % 100000))
    ...
    cfg["seed"] = int(seed)
    cfg["pseudo_scale"] = float(pseudo_scale)
    ...
    task_noise_scale = float(args.pseudo_noise_scale) * float(pseudo_scale)
    task_tail_boost = float(args.pseudo_tail_boost) * float(pseudo_scale)
    pseudo_x = make_pseudo(source_x, mu, sd, rng, task_noise_scale, task_tail_boost, pseudo_strategy)
    ...
    row = {
        ...
        "pseudo_setting": f"{pseudo_strategy}@scale={float(pseudo_scale):g}",
        "seed": int(seed),
        "device": int(device),
        "counts": {
...
def main():
    ...
    ap.add_argument("--pseudo_scale_grid", default="1.0")
    ...
    pseudo_scales = [float(x.strip()) for x in args.pseudo_scale_grid.split(",") if x.strip()]
    task_q = queue.Queue()
    settings = []
    for v in variants:
        for ps in pseudo_strategies:
            for scale in pseudo_scales:
                setting = f"{ps}@scale={scale:g}"
                if setting not in settings:
                    settings.append(setting)
                for s in seeds:
                    task_q.put((v, ps, scale, s))
    total = task_q.qsize()
    rows_by_setting = {setting: [] for setting in settings}
    errors = []
    done = 0
    lock = threading.Lock()
    ...
    def snapshot(status="running", current=None):
        ...
        for setting in settings:
            if rows_by_setting[setting]:
                partial.append({"pseudo_setting": setting, "summary": summarize(rows_by_setting[setting])})
    ...
    def worker(device):
        ...
        while True:
            try:
                variant, pseudo_strategy, pseudo_scale, seed = task_q.get_nowait()
            except queue.Empty:
                return
            setting = f"{pseudo_strategy}@scale={float(pseudo_scale):g}"
            current = {"variant": variant, "pseudo_strategy": pseudo_strategy, "pseudo_scale": float(pseudo_scale), "pseudo_setting": setting, "seed": seed, "device": int(device)}
            with lock:
                atomic_write_json(args.progress_out, snapshot("running", current))
            try:
                row = run_one(args, variant, pseudo_strategy, pseudo_scale, seed, int(device))
                with lock:
                ...
                with lock:
                    errors.append({"variant": variant, "pseudo_strategy": pseudo_strategy, "pseudo_scale": float(pseudo_scale), "pseudo_setting": setting, "seed": int(seed), "device": int(device), "error": repr(e), "traceback": tb})
                    done += 1
                    atomic_write_json(args.progress_out, snapshot("running", current))
                print(tb, flush=True)
            finally:
    ...
    payload = {
        "probe": "pseudo_g2_scale_calibration_probe",
        "codename": "C-MiT / G2 pseudo difficulty scale calibration",
        "protocol": {
            "reference_regime": "C-LEG3 old_exact_080_regime by default",
            "pseudo_generation": "multiple strategies and perturbation scales in response-matrix space, generated only from held-out known-normal matrices",
            "pseudo_strategies": pseudo_strategies,
            "pseudo_scale_grid": pseudo_scales,
        ...
        "errors": errors,
        "by_setting": {setting: {"rows": rows_by_setting[setting], "summary": summarize(rows_by_setting[setting]) if rows_by_setting[setting] else None} for setting in settings},
        "by_strategy": {
            ps: {
                "settings": {setting: {"rows": rows_by_setting[setting], "summary": summarize(rows_by_setting[setting]) if rows_by_setting[setting] else None} for setting in settings if setting.startswith(ps + "@")},
            } for ps in pseudo_strategies
        },
    }
