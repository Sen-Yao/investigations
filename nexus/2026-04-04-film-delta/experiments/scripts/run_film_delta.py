"""
FiLM-Delta VoxGFormer 训练脚本

功能：
1. 加载 Photo 数据集
2. 训练 FiLMDeltaVoxG 模型
3. 评估并对比基线
...
def evaluate(model, hop_features, labels, test_idx):
    ...
    probs = torch.sigmoid(scores[test_idx])
    preds = probs.cpu().numpy()
    targets = labels[test_idx].cpu().numpy()
    
    if len(np.unique(targets)) < 2:
        loss = nn.BCEWithLogitsLoss()(scores[test_idx], labels[test_idx].float())
...
def run_training(
    ...
    """完整训练流程"""
    print("=" * 60)
    print(f"FiLMDeltaVoxG Training (seed={seed})")
    print("=" * 60)
...
def run_5seed_experiment(seeds=[0, 1, 2, 3, 4], **kwargs):
    """5-seed 实验"""
    print("=" * 60)
    print("5-Seed Experiment: FiLMDeltaVoxG")
    print("=" * 60)
...
def main():
    ...
    print(f"{'DualStreamVoxG':<20} {'0.5157 ± 0.011':<20}")
    print(f"{'FiLMDeltaVoxG':<20} {f'{mean_auc:.4f} ± {std_auc:.4f}':<20}")
