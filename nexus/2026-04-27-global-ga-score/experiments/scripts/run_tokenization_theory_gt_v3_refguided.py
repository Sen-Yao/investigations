#!/usr/bin/env python3
"""GT Training with Reference-Guided Pseudo Anomaly Objective"""
...
# [Copy all helper functions from original script: set_seed, to_dense_features, l2_rows, rank_percentile, build_hop_attr, rwse, build_descriptor, NormalModel, normal_soft_or_score, cosine_rows_to_matrix, select_refs, apply_ablation, build_tokens, reference_purity, encode_tokens_batched, eval_logits]
# For brevity, inline the key ones:
...
class NormalModel:
    def __init__(self, estimator, z, normal_idx, pca_components=32):
        ...
        if estimator=="pca_residual":
            ...
            if ncomp>0: self.pca=PCA(n_components=ncomp,svd_solver="randomized",random_state=0); self.pca.fit(self.zs)
    def rejection(self):
...
def select_refs(z,residual,normal_idx,nm,features,adj,args,labels=None):
    ...
    return normal_refs, anom_refs, {"ga":ga}
...
def apply_ablation(normal_refs, anom_refs, normal_idx, labels, args):
    ...
    if args.ablation_mode=="no_ra": rep=np.resize(normal_refs, (n,args.anom_k)); return normal_refs, rep.astype(np.int64)
    if args.ablation_mode=="shuffled_ra": flat=anom_refs.copy(); rng.shuffle(flat, axis=0); return normal_refs, flat
    if args.ablation_mode=="fixed_labeled_normal": pool=np.asarray(normal_idx); fixed=np.resize(pool, args.normal_k); normal_fixed=np.tile(fixed[None,:],(n,1)); return normal_fixed.astype(np.int64), anom_refs
...
def main():
    ...
    ap.add_argument("--dataset",required=True); ap.add_argument("--device",type=int,default=0); ap.add_argument("--seed",type=int,default=0)
    ap.add_argument("--train_rate",type=float,default=0.05); ap.add_argument("--num_epoch",type=int,default=200)
    ap.add_argument("--descriptor_mode",default="hop_attr"); ap.add_argument("--pn_estimator",default="pca_residual")
    ap.add_argument("--gn_mode",default="label_gate"); ap.add_argument("--ln_mode",default="descriptor_similarity")
    ap.add_argument("--ga_mode",default="normal_rejection"); ap.add_argument("--la_mode",default="residual_cosine")
    ...
    ap.add_argument("--hops",type=int,default=2); ap.add_argument("--rw_steps",type=int,default=8); ap.add_argument("--pca_components",type=int,default=32)
    ap.add_argument("--lr",type=float,default=1e-3); ap.add_argument("--weight_decay",type=float,default=0.0)
    ap.add_argument("--pseudo_beta",type=float,default=0.3); ap.add_argument("--pseudo_noise",type=float,default=0.01)
    ap.add_argument("--embedding_dim",type=int,default=256); ap.add_argument("--GT_ffn_dim",type=int,default=256); ap.add_argument("--GT_dropout",type=float,default=0.4); ap.add_argument("--GT_attention_dropout",type=float,default=0.4); ap.add_argument("--GT_num_heads",type=int,default=2); ap.add_argument("--GT_num_layers",type=int,default=3); ap.add_argument("--sample_rate",type=float,default=0.15); ap.add_argument("--mean",type=float,default=0.02); ap.add_argument("--var",type=float,default=0.01); ap.add_argument("--outlier_beta",type=float,default=0.3); ap.add_argument("--ring_R_max",type=float,default=1.0); ap.add_argument("--ring_R_min",type=float,default=0.3); ap.add_argument("--lambda_rec_tok",type=float,default=1.0); ap.add_argument("--lambda_rec_emb",type=float,default=0.1); ap.add_argument("--encode_batch_size",type=int,default=2048)
    ap.add_argument("--wandb",type=lambda x: str(x).lower() in ["1","true","yes"],default=False)
    ap.add_argument("--out",default="")
    ap.add_argument("--objective_mode",default="ref_guided")  # NEW: ref_guided or self_residual
    ...
    assert np.sum(labels_np[normal_idx])==0, "Data leakage!"
    ...
    # Collect anom_ref indices for ref_guided mode
    anom_ref_flat = list(set(anom_refs.flatten().tolist()))
    normal_ref_flat = list(set(normal_refs.flatten().tolist()))
    anom_ref_t = torch.tensor(anom_ref_flat, dtype=torch.long, device=device)
    normal_ref_t = torch.tensor(normal_ref_flat, dtype=torch.long, device=device)
    ...
    for epoch in range(args.num_epoch+1):
        ...
        if epoch%10==0 or epoch==args.num_epoch:
            ...
            print(json.dumps({"epoch":epoch,"val_auc":val_auc,"test_auc":test_auc,"best_epoch":best["epoch"]},ensure_ascii=False),flush=True)
    
    elapsed=time.time()-start_time
    result={"dataset":args.dataset,"objective_mode":args.objective_mode,"best":best,"purity":pur,"elapsed_sec":elapsed}
    print("FINAL",json.dumps(result,indent=2,ensure_ascii=False))
    if args.out: Path(args.out).parent.mkdir(parents=True,exist_ok=True); Path(args.out).write_text(json.dumps(result,indent=2,ensure_ascii=False))
