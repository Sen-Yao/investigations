...
def main():
    ...
    ap.add_argument('--encode_batch_size',type=int,default=2048); ap.add_argument('--wandb',type=lambda x: str(x).lower() in ['1','true','yes'],default=False); ap.add_argument('--dry_run',action='store_true'); ap.add_argument('--out',default=''); ap.add_argument('--objective_mode',default='ref_guided')
    ...
    if args.wandb:
        import wandb
        wandb_run=wandb.init(project='VoxG', entity='HCCS', config=vars(args), name=f"tokgt_{args.dataset}_{args.descriptor_mode}_{args.pn_estimator}_{args.ga_mode}_{args.ablation_mode}")
    ...
    for epoch in range(args.num_epoch+1):
        ...
        # Objective mode
        if hasattr(args,'objective_mode') and args.objective_mode=='self_residual':
            center=normal_emb.mean(dim=0,keepdim=True); direction=F.normalize(normal_emb-center,dim=1); outlier_emb=normal_emb+args.pseudo_beta*direction+torch.randn_like(normal_emb)*args.pseudo_noise
        else:
            anom_ref_flat=list(set(anom_refs.flatten().tolist())); normal_ref_flat=list(set(normal_refs.flatten().tolist()))
            anom_ref_t=torch.tensor(anom_ref_flat,dtype=torch.long,device=device); normal_ref_t=torch.tensor(normal_ref_flat,dtype=torch.long,device=device)
            pool_ra=emb[anom_ref_t].mean(dim=0); pool_rn=emb[normal_ref_t].mean(dim=0)
        ...
        if epoch%10==0 or epoch==args.num_epoch:
            ...
            row={'epoch':epoch,'loss':float(loss.item()),'val_auc':val_auc,'val_ap':val_ap,'test_auc':test_auc,'test_ap':test_ap,'best_val_auc':best['val_auc'],'best_val_ap':best['val_ap'],'best_test_auc':best['test_auc'],'best_test_ap':best['test_ap']}
            print(json.dumps(row,ensure_ascii=False),flush=True)
            if wandb_run: wandb.log(row,step=epoch)
    result={'dataset':args.dataset,'seed':args.seed,'config':vars(args),'best':best,'purity':pur,'time_sec':time.time()-start}
    print('FINAL',json.dumps(result,indent=2,ensure_ascii=False))
    if wandb_run:
        wandb.summary['best_val_auc']=best['val_auc']; wandb.summary['best_val_ap']=best['val_ap']; wandb.summary['test_auc']=best['test_auc']; wandb.summary['test_ap']=best['test_ap']; wandb.finish()
    if args.out:
