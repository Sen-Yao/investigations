#!/usr/bin/env python3
"""
Hop-Offset vs Delta 信息熵分析 v2 - 对称归一化版
===================================================

关键修改（相对于 v1）：
1. 归一化方式：D^(-0.5) @ A @ D^(-0.5)（对称归一化）
2. alpha = 0.5（默认值）

任务：
- 计算三个数据集（Photo, Elliptic, Tolokers）的：
  - Hop-Offset 信息熵（对称归一化）
  - Delta 信息熵（对称归一化）
- 对比两者的信息量差异
"""
...
def compute_hop_offset_features_v2(adj, features, pp_k=10, alpha=0.5):
    """
    计算 Hop-Offset 特征 (使用对称归一化)
    
    关键区别于 v1：
    - 归一化：D^(-0.5) @ A @ D^(-0.5) (对称归一化，不含 self-loop)
    - 传播方式：X^{(k+1)} = (1-alpha) * A_sym * X^{(k)} + alpha * X^{(0)}
    ...
    """
    # 对称归一化（不含 self-loop）
    adj_sym = symmetric_normalize_adj(adj)
    adj_tensor = torch.FloatTensor(np.array(adj_sym.todense()))
    ...
    x_0 = features_tensor  # 初始特征用于残差连接
    
    for hop in range(pp_k):
        # 对称归一化传播 + 残差连接
        # X^{(k+1)} = (1-alpha) * A_sym * X^{(k)} + alpha * X^{(0)}
...
def analyze_dataset(dataset_name, pp_k=10, alpha=0.5):
    """分析单个数据集"""
    print(f"\n{'='*60}")
    ...
    # 计算 Hop-Offset 特征（对称归一化）
    print(f"\n计算 Hop-Offset 特征（对称归一化: D^-0.5 @ A @ D^-0.5, alpha={alpha}, pp_k={pp_k})...")
    hop_offset_features = compute_hop_offset_features_v2(adj, features, pp_k=pp_k, alpha=alpha)
...
def main():
    """主函数"""
    print("="*60)
    print("Hop-Offset vs Delta 信息熵分析 v2")
    print("="*60)
    print("归一化方式: D^(-0.5) @ A @ D^(-0.5) (对称归一化)")
    print("配置: alpha=0.5, pp_k=10")
    ...
    for dataset in datasets:
        try:
            result = analyze_dataset(dataset, pp_k=10, alpha=0.5)
    ...
    # 保存结果到文件
    output_path = '/root/gpufree-data/linziyao/VoxG/nexus/investigations/2026-03-30-hop-offset-analysis/experiments/outputs/entropy_results_v2.txt'
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    
    with open(output_path, 'w') as f:
        f.write("Hop-Offset vs Delta 信息熵分析 v2（对称归一化）\n")
        f.write("="*60 + "\n")
        f.write("归一化方式: D^(-0.5) @ A @ D^(-0.5)\n")
        f.write("配置: alpha=0.5, pp_k=10\n\n")
