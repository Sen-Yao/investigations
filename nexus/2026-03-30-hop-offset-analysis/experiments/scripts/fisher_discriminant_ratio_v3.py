#!/usr/bin/env python3
"""
Fisher 判别比分析 v3 - 对称归一化 + alpha=0.1 残差

关键参数：
- 归一化：D^(-0.5) @ A @ D^(-0.5)（对称归一化）
- alpha=0.1（残差系数，保留原始特征比例）

传播公式：
- Hop_0 = X
- Hop_k = (1-alpha) * A_sym @ Hop_{k-1} + alpha * X

分析内容：
1. 各 hop 的 Fisher 判别比
...
def compute_hops_with_residual(attr, network, sample_idx, pp_k=10, alpha=0.1):
    """
    计算各 hop 的特征（使用对称归一化 + alpha 残差）
    ...
    # Hop 0（原始特征）
    hop0 = attr_dense[sample_idx]
    
    # 计算 Hop 1 到 pp_k（带残差传播）
    hops = [hop0]
    prev_hop = attr_dense  # 全量特征，用于传播
    
    for k in range(1, pp_k + 1):
        # 传播：A_sym @ prev_hop
        propagated = norm_adj @ prev_hop
        if sp.issparse(propagated):
            propagated = propagated.toarray()
        
        # 残差连接：(1-alpha) * propagated + alpha * X
        current_hop = (1 - alpha) * propagated + alpha * attr_dense
        
        # 提取采样节点
        hop_k = current_hop[sample_idx]
        hops.append(hop_k)
...
def compute_fisher_ratio(normal_values, anomaly_values):
    ...
    var_n = np.var(normal_values, ddof=1)
...
def analyze_dataset(dataset_name, sample_size=2000, pp_k=10, alpha=0.1):
    """分析单个数据集"""
    print(f"\n{'='*80}")
    ...
    print(f"参数: 对称归一化 (D^-0.5 @ A @ D^-0.5), alpha={alpha}, pp_k={pp_k}")
    
    # 计算各 hop 特征
    print(f"\n计算各 Hop 特征（带 alpha={alpha} 残差）...")
    hops = compute_hops_with_residual(attr, network, sample_idx, pp_k=pp_k, alpha=alpha)
...
def main():
    print("=" * 80)
    print("Fisher 判别比分析 v3 - 对称归一化 + alpha=0.1 残差")
    print("=" * 80)
    ...
    for dataset in datasets:
        result = analyze_dataset(dataset, sample_size=sample_size, pp_k=pp_k, alpha=alpha)
        if result:
