...
Fisher 判别比 = (μ_normal - μ_anomaly)^2 / (σ_normal^2 + σ_anomaly^2)
...
def compute_hops(attr, network, sample_idx, pp_k=10, alpha=0.0):
    """计算各 hop 的特征"""
    # 归一化邻接矩阵
...
def compute_fisher_ratio(normal_values, anomaly_values):
    """
    计算 Fisher 判别比
    F = (μ_normal - μ_anomaly)^2 / (σ_normal^2 + σ_anomaly^2)
    """
...
def analyze_dataset(dataset_name, sample_size=2000, pp_k=10, alpha=0.0):
    """分析单个数据集"""
    print(f"\n{'='*80}")
    ...
    # 计算各 hop 特征
    print(f"\n计算各 Hop 特征...")
    hops = compute_hops(attr, network, sample_idx, pp_k=pp_k, alpha=alpha)
...
def main():
    print("=" * 80)
    print("Fisher 判别比分析 - Hop vs Hop-Offset vs Delta")
    print("=" * 80)
    ...
    # 保存结果
    output_path = '/root/gpufree-data/linziyao/VoxG/nexus/investigations/2026-03-30-hop-offset-analysis/experiments/outputs/fisher_discriminant_results.json'
