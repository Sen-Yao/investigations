...
def main():
    ...
    ap.add_argument('--diagnosis_mode', choices=['train_all','freeze_encoder','train_head_only'], default='train_all')
    ...
    if args.wandb:
        import wandb
        wandb_run=wandb.init(project='VoxG', entity='HCCS', config=vars(args), name=f"diag_degrade_{args.dataset}_{args.diagnosis_mode}_{args.objective_mode}_b{args.pseudo_beta}_s{args.seed}")
        wandb.summary.update(pur)
    model=VecGAD(features_np.shape[1],args.embedding_dim,'prelu',args).to(device)
    if args.diagnosis_mode in ['freeze_encoder','train_head_only']:
        for name,p in model.named_parameters():
            if not (name.startswith('fc1') or name.startswith('fc2') or name.startswith('fc3')):
                p.requires_grad=False
    opt=torch.optim.Adam([p for p in model.parameters() if p.requires_grad],lr=args.lr,weight_decay=args.weight_decay); bce=nn.BCEWithLogitsLoss(); normal_t=torch.tensor(normal_idx,dtype=torch.long,device=device)
    model.eval()
    with torch.no_grad():
        emb0=encode_tokens_batched(model,token_tensor,device,args.encode_batch_size).detach()
        ref_score0=torch.linalg.norm(emb0-torch.mean(emb0[normal_t],dim=0,keepdim=True),dim=1).detach().cpu().numpy()
        logits0=scorer(model,emb0).detach().cpu().numpy()
    best={'val_auc':-1,'val_ap':-1,'test_auc':-1,'test_ap':-1,'epoch':-1}; start=time.time()
    for epoch in range(args.num_epoch+1):
        model.train(); opt.zero_grad(); enc_grad = args.diagnosis_mode == 'train_all'
        ...
        emb_c=torch.cat([normal_emb,outlier_emb],dim=0); logits_train=scorer(model,emb_c); y=torch.cat([torch.zeros(len(normal_emb),device=device),torch.ones(len(outlier_emb),device=device)])
        loss=bce(logits_train,y); loss.backward(); opt.step()
        if epoch%10==0 or epoch==args.num_epoch:
            model.eval()
            with torch.no_grad(): emb_eval=encode_tokens_batched(model,token_tensor,device,args.encode_batch_size); logits=scorer(model,emb_eval).detach().cpu().numpy()
            ...
            drift=float(torch.linalg.norm(emb_eval-emb0,dim=1).mean().item())
            pseudo_y=np.r_[np.zeros(len(normal_emb)),np.ones(len(outlier_emb))]
            pseudo_auc,pseudo_ap=eval_logits(logits_train.detach().cpu().numpy(),pseudo_y,np.arange(len(pseudo_y)))
            row={'epoch':epoch,'loss':float(loss.item()),'val_auc':val_auc,'val_ap':val_ap,'test_auc':test_auc,'test_ap':test_ap,'AUC':test_auc,'AP':test_ap,'best_val_auc':best['val_auc'],'best_val_ap':best['val_ap'],'best_test_auc':best['test_auc'],'best_test_ap':best['test_ap'], 'diagnosis_mode':args.diagnosis_mode, 'emb_drift_mean':drift, 'spearman_model_label':safe_spearman(logits[idx_test],labels_np[idx_test]), 'spearman_model_ref0':safe_spearman(logits,ref_score0), 'spearman_model0_model':safe_spearman(logits,logits0), 'pseudo_auc':pseudo_auc, 'pseudo_ap':pseudo_ap}
