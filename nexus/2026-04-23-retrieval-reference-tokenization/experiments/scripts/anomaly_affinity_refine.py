#!/usr/bin/env python3
"""
Refine single-stage anomaly affinity score with several elegant variants.
"""
...
def build_components(ndc, adj):
    q = sigmoid((ndc - ndc.mean()) / (ndc.std() + 1e-8))
    n = len(q)
    c = np.zeros(n, dtype=np.float32)
    c_max = np.zeros(n, dtype=np.float32)
    for i in range(n):
        neighbors = np.where(adj[i] > 0)[0]
        if len(neighbors) == 0:
            c[i] = q[i]
            ...
            c_max[i] = float(np.max(q[neighbors]))
    return q, c, c_max
...
def main():
    ...
    q, c, c_max = build_components(ndc, adj)
    ...
    variants = {
        ...
        'local_mean_c': c,
        'affinity_mul': q * c,
        'affinity_geom': np.sqrt(np.clip(q * c, 0.0, 1.0)),
        'affinity_hmean': 2 * q * c / (q + c + 1e-8),
        'affinity_maxblend': 0.5 * q + 0.5 * c_max,
    }
