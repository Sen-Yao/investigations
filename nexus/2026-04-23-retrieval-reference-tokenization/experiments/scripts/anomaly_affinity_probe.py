#!/usr/bin/env python3
"""
Single-stage anomaly affinity score probe.
q(v): NDC-based anomaly prior
c(v): local concentration of q over neighbors
s(v)=q(v)*c(v)
"""
...
def build_affinity_score(ndc, adj):
...
def main():
    ...
    q, c, s = build_affinity_score(ndc, adj)
    ...
    result = {
        ...
        'anomaly_prior_q': eval_score(q, labels, args.topk),
        'local_concentration_c': eval_score(c, labels, args.topk),
        'affinity_score': eval_score(s, labels, args.topk),
    }
