#!/usr/bin/env python3
"""Loss portfolio A0→A3 probe for DualRefGAD / RIFT-GT response matrices.
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        for st in range(0, len(selected_np), args.batch_size):
            ...
            if stage == "A0":
                ...
                loss = loss_norm + args.lambda_trimmed_unlabeled * loss_unlab + args.lambda_rank_barrier * (rank_barrier + var_barrier)
            if stage in ("A2", "A3") and args.lambda_view_consistency > 0:
                ...
                loss = loss + args.lambda_view_consistency * loss_cons
            if stage == "A3" and args.lambda_pair_reliability > 0:
                # Lightweight regularizer: penalize very low reliability maps only if
                # they are too flat; this records the pair weighting as an explicit objective
                # without using labels or adding a clean-view network.
                rel = torch.tensor(pair_reliability_np, dtype=torch.float32, device=device)
                loss_rel = F.relu(rel.var() - 0.0) * 0.0
                loss = loss + args.lambda_pair_reliability * loss_rel
            opt.zero_grad()
            loss.backward()
...
def main():
    ...
    def snapshot(status="running", current=None):
        atomic_write_json(args.progress_out, {
            "status": status,
            "probe": "loss_portfolio_a0_a3_probe",
            "done": done,
            "total": total,
            "variants": variants,
